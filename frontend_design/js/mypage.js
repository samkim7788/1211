...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnConfirmDeleteAccount.addEventListener('click', () => {
        ...
        // Clear all data
        localStorage.removeItem('moodon_isLoggedIn');
        localStorage.removeItem('moodon_userEmail');
        localStorage.removeItem('moodon_userPreferences');
        localStorage.removeItem('moodon_favoriteProducts');
        localStorage.removeItem('moodon_chat_sessions');
    });
        ...
        if (favoriteProducts.length === 0) {
            favoritesGrid.innerHTML = `
                <div class="h-full flex flex-col items-center justify-center">
                    ...
                    <button onclick="window.location.href='chat.html'" class="px-8 py-4 bg-gradient-to-r from-blue-500 to-blue-400 text-white rounded-2xl hover:from-blue-600 hover:to-blue-500 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center gap-2">
                        <span>챗봇으로 가기</span>
                        <span>💬</span>
                    </button>
                </div>
